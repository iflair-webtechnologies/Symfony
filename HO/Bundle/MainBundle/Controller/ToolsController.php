...
class ToolsController extends Controller
{
    ...
    public function indexAction()
    {
        return array(
            'tabs' => $this->getTabs(),
        	'leftmenu' => $this->getTabs()
        );
    }

    /**
     * @Route("/tools/checkfeeds")
     * @Template()
     */
    public function checkFeedsAction()
    {
        ...
        $tools = $em->getRepository('MainBundle:tools')->findAll();

        return array('tools' => $tools);
    }

    /**
     * @Route("/tools/addfeeds")
     */
    public function addFeedAction()
    {
        ...
        $statement = $connection->prepare('insert into tools (title,client_id,url,project_id) values("'.$request->query->get('t').'","'.$request->query->get('c').'","'.$request->query->get('u').'","'.$request->query->get('p').'")');
        ...
        exit;
        $up = new tools();
        $up->setTitle($request->query->get('t'));
        $up->setClientId($request->query->get('c'));
        $up->setUrl($request->query->get('u'));
        $up->setProjectId($request->query->get('p'));
     ...
     * @param Request $request
     * @param $id
     * @Route("/tools/user/{id}/changepassword")
     * @Template()
     * @return array
     */
